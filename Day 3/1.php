...
$validation_1 = explode("\n", "R75,D30,R83,U83,L12,D49,R71,U7,L72
U62,R66,U55,R34,D71,R55,D58,R83");

$validation_2 = explode("\n", "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51
...
foreach ($input as $path_id => $path) {
	$paths[$path_id] = [[0,0,0]];
	$steps = 1;

	foreach (explode(",", $path) as $movement) {
		$position = end($paths[$path_id]);
		$distance = (int) substr($movement, 1);

		switch (substr($movement, 0, 1)) {
			case 'U':
				while (($position[0] - $distance) < end($paths[$path_id])[0]) {
					$paths[$path_id][] = [end($paths[$path_id])[0]-1, $position[1], $steps++];
				}
				break;

			case 'L':
				while (($position[1] - $distance) < end($paths[$path_id])[1]) {
					$paths[$path_id][] = [$position[0], end($paths[$path_id])[1]-1, $steps++];
				}
				break;
...
$distances = [];
$wire_steps = [];
foreach ($paths[0] as $coordinate_0) {
	foreach ($paths[1] as $coordinate_1) {
		if ($coordinate_0[0] === $coordinate_1[0] && $coordinate_0[1] === $coordinate_1[1]) {
			if ($coordinate_0[0] !== 0 && $coordinate_0[1] !== 0) {
				$distances[] = manhattan_distance($coordinate_0, [0,0]);
				$wire_steps[] = $coordinate_0[2] + $coordinate_1[2];
			}
		}
	}
}
...
print "Solution 2: " . min($wire_steps) . "\n";

function manhattan_distance($coordinate, $targetCoordinate): int {
	$steps = [0];

	for ($i = 0; $i < 2; $i++) {
		$steps[] = abs($coordinate[$i] - $targetCoordinate[$i]);
	}
